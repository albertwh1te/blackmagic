...
def SolarFromInt(g):
    ...
    solar = datetime.datetime(y, mm, dd).date()
    return solar
...
class TraditionConverter:
    ...
    def tradition_to_normal(self, normal_date):
        lunar = Lunar(normal_date.year, normal_date.month, normal_date.day, False)
        days = TraditionConverter.lunar_month_days[lunar.lunarYear - TraditionConverter.lunar_month_days[0]]
        ...
        solar11 = TraditionConverter.solar_1_1[lunar.lunarYear - TraditionConverter.solar_1_1[0]]
        y = GetBitInt(solar11, 12, 9)
        m = GetBitInt(solar11, 4, 5)
        d = GetBitInt(solar11, 5, 0)
        return SolarFromInt(SolarToInt(y, m, d) + offset - 1)

    def normal_to_tradition(self, tradition_date):
        solar = Solar(tradition_date.year, tradition_date.month, tradition_date.day)
        lunar = Lunar(0, 0, 0, False)
        index = solar.solarYear - TraditionConverter.solar_1_1[0]
        data = (solar.solarYear << 9) | (solar.solarMonth << 5) | solar.solarDay
        if TraditionConverter.solar_1_1[index] > data:
            index -= 1
        solar11 = TraditionConverter.solar_1_1[index]
        y = GetBitInt(solar11, 12, 9)
        m = GetBitInt(solar11, 4, 5)
        d = GetBitInt(solar11, 5, 0)
        offset = SolarToInt(solar.solarYear, solar.solarMonth, solar.solarDay) - SolarToInt(y, m, d)
        days = TraditionConverter.lunar_month_days[index]
        leap = GetBitInt(days, 4, 13)
        lunarY = index + TraditionConverter.solar_1_1[0]
        lunarM = 1
        offset += 1
        for i in range(0, 13):
        ...
        lunar = datetime.datetime(lunarY, lunarM, lunarD, False).date()
        return lunar
